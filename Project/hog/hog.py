...
def boar_brawl(player_score, opponent_score):
    ...
    if tens_digit != ones_digit :
        if opponent_score < 10 :
            score = abs(3*(0-ones_digit))
        else :
            score = abs(3*(tens_digit-ones_digit))
    else  :
        score = 1
    return score
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    start_value = strategy(0,0)
    score = 0
    while score < goal : 
        opponent_score = 0 
        while opponent_score < goal :
            if strategy (score,opponent_score) != start_value :
                return False
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for num_rolls in range(1,11) :
        current_average = make_averaged(roll_dice,times_called)(num_rolls,dice)
        if current_average > best_average_score :
            best_average_score = current_average
